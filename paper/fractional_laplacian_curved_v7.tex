...
\title{\textbf{Fractional Laplacians on Curved Backgrounds:}\\
...
Integrating over $\M$ and using the Sobolev embedding $H^2(\M) \hookrightarrow C^0(\M)$ for $d=2$:
...
\item $R^{-2}$ scaling exponent: $-2.0000 \pm 10^{-15}$ (machine-precision confirmation of exact dimensional analysis)
\item Correction signs: All 51 modes show correct sign (100\% sign accuracy); maximum magnitude error 0.589\% at $\ell=10$
\end{itemize}
...
\begin{itemize}
\item \textbf{GitHub repository}: \url{https://github.com/boonespacedog/Fractional-Laplacian}
  \begin{itemize}
  \item experiments/E53\_sphere\_validation/ - Sphere curvature correction validation (< 1 second runtime)
  \end{itemize}
\item \textbf{Zenodo archive}: DOI \texttt{10.5281/zenodo.17585575} (permanent archival copy)
